...
class KodikPlayer extends PlayerBase {
  ...
  rebuildIFrameSrc() {
    let src = new URL(`https:${this._translation.iframeUrl}`);
...
class KodikApi {
  ...
  async search(shikimoriId, abort) {
    // Получаем Kinopoisk ID со страницы
    const kpLink = document.querySelector("a[href*='kinopoisk']");

    const kinopoiskId = kpLink
      ? kpLink.href.match(/\/film\/(\d+)/)?.[1] ||
        kpLink.href.match(/\/series\/(\d+)/)?.[1]
      : null;
    ...
    // Возвращаем в формате старого Kodik
    return [
      {
...
async function startShikiplayer() {
  if (location.hostname !== "shikimori.io") return;
